...
PF_BOOT_CLASSPATH="$pfrunjar:$PF_HOME/startup/*:$xmlbeans:$pfxml"
...
if [[ "$JAVA_PRODUCT_VERSION" -eq "1" ]]; then
  JAVA_PRODUCT_VERSION=$(echo ${JAVA_VERSION/_/.} | cut -d. -f 2)
fi
...
trap 'kill $PID; wait $PID; cat </dev/null 2>/dev/null >$RUNFILE' 1 2 3 6 15
